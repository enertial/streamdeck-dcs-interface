...
void StreamdeckContext::updateContextSettings(const json &settings) {
    ...
    const bool compare_monitor_is_populated =
        (!dcs_id_compare_monitor_raw.empty() && is_integer(dcs_id_compare_monitor_raw));
    const bool comparison_value_is_populated =
        (!dcs_id_comparison_value_raw.empty() && is_number(dcs_id_comparison_value_raw));

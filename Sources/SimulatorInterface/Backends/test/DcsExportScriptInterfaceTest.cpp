...
TEST(DcsExportScriptInterfaceTest, invalid_connection_port_settings)
{
    ...
    EXPECT_THROW(DcsExportScriptInterface dcs_interface(connection_settings), std::runtime_error);
}
...
TEST_F(DcsExportScriptInterfaceTestFixture, update_simulator_state)
{
    ...
    EXPECT_EQ("1", dcs_interface.get_value_of_simulator_object_state(761).value());
    EXPECT_EQ("2.00", dcs_interface.get_value_of_simulator_object_state(765).value());
    EXPECT_EQ("TEXT_STR", dcs_interface.get_value_of_simulator_object_state(2026).value());
    EXPECT_EQ("4", dcs_interface.get_value_of_simulator_object_state(2027).value());
    ...
    EXPECT_EQ("99.99", dcs_interface.get_value_of_simulator_object_state(765).value());
...
TEST_F(DcsExportScriptInterfaceTestFixture, send_simulator_command)
{
    ...
    dcs_interface.send_simulator_command(button_id, device_id, value);
...
{
    dcs_interface.send_simulator_reset_command();
    std::stringstream ss_received = mock_dcs.receive();
...
TEST_F(DcsExportScriptInterfaceTestFixture, get_current_simulator_module_init)
{
    ...
    EXPECT_EQ("", dcs_interface.get_current_simulator_module());
}
...
TEST_F(DcsExportScriptInterfaceTestFixture, get_current_simulator_module)
{
    ...
    mock_dcs.send(mock_dcs_message);
    dcs_interface.update_simulator_state();
    EXPECT_EQ("AV8BNA", dcs_interface.get_current_simulator_module());
...
{
    EXPECT_FALSE(dcs_interface.get_value_of_simulator_object_state(999));
}
...
TEST_F(DcsExportScriptInterfaceTestFixture, get_value_of_simulator_object_state_with_valid_value)
{
    ...
    auto maybe_value = dcs_interface.get_value_of_simulator_object_state(765);
...
{
    EXPECT_FALSE(dcs_interface.get_decimal_of_simulator_object_state(999));
}
...
TEST_F(DcsExportScriptInterfaceTestFixture, get_decimal_of_simulator_object_state_with_valid_value)
{
    ...
    auto maybe_value = dcs_interface.get_decimal_of_simulator_object_state(765);
    EXPECT_TRUE(maybe_value);
